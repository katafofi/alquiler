    ...
    useEffect(() => {
        ...
        if (window.prompt("Ingrese la credencial de autorizacion", 0) == "202312") {
          if (window.confirm("¿Estás seguro de que quieres eliminar?")) {
            try {
              ...
              setForm((prev) => prev.filter((info) => info.IdCategoria != id));
              setDeleted(true);
              if (selected && selected.IdCategoria == id) {
                setSelected(null);
                setNews({
                  IdCategoria: "",
                  Descripcion: ""
      ...
      const handleUpdate = async () => {
        const response = await fetch(
          `${URL}${PORT}/${form}/${selected.IdCategoria}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
        ...
        setForm((prev) =>
          prev.map((estado) =>
            estado.IdCategoria == data.IdCategoria ? data : estado
          )
        );
        setSelected(null);
        setNewEmploye({
