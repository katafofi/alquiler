    ...
    useEffect(() => {
        ...
        if (window.prompt("Ingrese la credencial de autorizacion", 0) == "202312") {
          if (window.confirm("¿Estás seguro de que quieres eliminar?")) {
            try {
              ...
              setForm((prev) => prev.filter((info) => info.IdEstadoPago != id));
              setDeleted(true);
              if (selected && selected.IdEstadoPago == id) {
                setSelected(null);
                setNews({
                  IdEstadoPago: "",
                  descripcion: ""
      ...
      const handleUpdate = async () => {
        const response = await fetch(
          `${URL}${PORT}/${form}/${selected.IdEstadoPago}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
        ...
        setForm((prev) =>
          prev.map((estado) =>
            estado.IdEstadoPago == data.IdEstadoPago ? data : estado
          )
        );
        setSelected(null);
        setNewEmploye({
